"""
train_random_forest_regressor.py

This script handles training a Random Forest model for forecasting.
Main tasks:
  - Define Optuna objective for hyperparameter search
  - TPE search to find the best hyperparameter
  - Train a final Random Forest model with the best parameters
...
def run_tpesampler_random_forest(
  X_train: np.ndarray,
  y_train: np.ndarray,
  X_val: np.ndarray,
  y_val: np.ndarray,
  ...
  """Run exhaustive hyperparameter search for Random Forest regressor using Optuna.
  ...
  Args:
    ...
    X_val (np.ndarray): Validation features.
    y_val (np.ndarray): Validation targets.
    error_metric (str): Error metric ('mse' or 'mae').
  ...
  objective_func = lambda trial: random_forest_objective(
      trial,
      X_train,
      y_train,
      X_val,
...
def train_random_forest_regressor(
  ...
  Args:
    ...
    experiment_name (str): WandB experiment name.
  ...
  model_path = os.path.join(model_dir, 'random_forest_regressor_model.joblib')
  study_path = os.path.join(study_dir, 'random_forest_regressor_study.joblib')

  # Load existing model/study if available
  if os.path.exists(model_path) and os.path.exists(study_path):
    logging.info(f"Using existing Random Forest regressor model: {model_path}")
    ...
    logging.info('Starting hyperparameter search for Random Forest regressor...')
    start_time_hyperparameter_search = time.time()

    study = run_tpesampler_random_forest(
      X_train,
      y_train,
      X_val,
      y_val,
    ...
    logging.info(f"Best parameters: "
      f"{study.best_params['n_estimators']=} {study.best_params['max_depth']=}, "
      f"{study.best_params['min_samples_split']=}, {study.best_params['min_samples_leaf']=}"
    )
    ...
    logging.info(f"Saving final Random Forest model to {model_path} and study to {study_path}.")
    ...
    if wandb_bool:
      run_name = f"{experiment_name}_random_forest_regressor_model_configs"
      with wandb.init(
        project = project_name,
        group = experiment_name,
        name = run_name,
        job_type = 'model_configs',
        tags = [project_name, 'random_forest_regressor_model', 'model_configs', 'prediction'],
        notes = f"Configurations of the final Random Forest regressor model for {experiment_name}."
      ) as run:
        run.log({
        ...
        model_artifact = wandb.Artifact(
            name = f"{experiment_name}_random_forest_regressor_model",
            type = 'model',
            metadata = {
                'best_n_estimators': study.best_params['n_estimators'],
                'best_max_depth': study.best_params['max_depth'],
        ...
        study_artifact = wandb.Artifact(
            name = f"{experiment_name}_random_forest_regressor_study",
            type = 'study',
            metadata = {
                'best_n_estimators': study.best_params['n_estimators'],
                'best_max_depth': study.best_params['max_depth'],
