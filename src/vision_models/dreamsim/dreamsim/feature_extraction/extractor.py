...
class ViTExtractor(nn.Module):
    ...
    def create_model(model_type: str, load_dir: str = "./models") -> nn.Module:
        ...
        # --- NEW: DINOv3 B/16 (build locally with timm, load local weights) ---
        if model_type == 'dinov3_vitb16':
            model = timm.create_model('vit_base_patch16_224', pretrained=False)
            ckpt_path = os.path.join(load_dir, 'dinov3_vitb16_pretrain.pth')
            if not os.path.exists(ckpt_path):
                raise FileNotFoundError(f"Expected DINOv3 weights at {ckpt_path}")
