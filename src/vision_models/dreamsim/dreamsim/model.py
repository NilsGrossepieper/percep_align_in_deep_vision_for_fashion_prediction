...
This module implements a ViT-based perceptual distance model (“DreamSim”-style).
It wraps one or more ViT backbones (e.g., DINO, CLIP, OpenCLIP, MAE), extracts
features (CLS / last_layer / embedding / CLS+patch), concatenates them, and
optionally applies a small residual MLP head. The model returns a cosine
distance between two images.

Main tasks:
  - Build ViT feature extractors from local checkpoints under:
      <project_root>/vision_models/vits_backbone_models
  - Define per-model feature extraction (CLS / last_layer / embedding / cls_patch)
  - Concatenate features across models and pass through an optional residual MLP
  - Normalize embeddings (per-sample mean removal + L2) for stable distances
  - Expose:
      • embed(img):      returns an image embedding
      • forward(a, b):   returns 1 − cosine_similarity(embed(a), embed(b))

Notes:
  - Inputs to `embed`/`forward` are 3×H×W tensors in [0,1]; model applies the
    appropriate normalization internally per backbone family.
  - If `feat_type` is 'cls_patch', patch tokens are spatially avg-pooled and
    concatenated with the CLS token.
  - Device is handled via `device` at init; module and inputs are moved accordingly.
"""
import json
import os
import zipfile
...
class PerceptualModel(torch.nn.Module):
    def __init__(self,
        ...
        load_dir = os.path.join(project_root, "vision_models", "vits_backbone_models")
...
def download_weights(cache_dir, dreamsim_type):
    ...
    dreamsim_required_ckpts = {
        ...
        "dinov3_vitb16": ["dinov3_vitb16_pretrain.pth", "dinov3_vitb16_single_lora"],
...
EMBED_DIMS = {
    ...
    'dinov3_vitb16': {'cls': 768, 'last_layer': 768},
